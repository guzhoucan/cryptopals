#ifndef CRYPTOPALS_AES_BASE_H_
#define CRYPTOPALS_AES_BASE_H_

#include <cstdint>

namespace cryptopals::aes {

// GF(2^8) irreducible polynomial x^8 + x^4 + x^3 + x + 1 used by AES.
inline constexpr uint32_t kPoly =
    1u << 8u | 1u << 4u | 1u << 3u | 1u << 1u | 1u;

// Power of x mod poly in GF(2^8)
// i.e. kPowX[i] == {0x01} * {1 << i}
// Check FIPS-197 4.2.1 Multiplication by x, and UT
inline constexpr uint8_t kPowX[] = {
    0x01,  // 1
    0x02,  // x
    0x04,  // x^2
    0x08,  // x^3
    0x10,  // x^4
    0x20,  // x^5
    0x40,  // x^6
    0x80,  // x^7
    0x1b,  // x^8
    0x36,  // x^9
    0x6c,  // x^10
    0xd8,  // x^11
    0xab,  // x^12
    0x4d,  // x^13
    0x9a,  // x^14
    0x2f   // x^15
};

// Multiply a and b as GF(2^8) polynomials modulo kPoly
uint8_t Mul(uint8_t a, uint8_t b);

// FIPS-197 Figure 7. S-box substitution values in hexadecimal format.
inline constexpr uint8_t kSBox0[] = {
    ,
};

// FIPS-197 Figure 14.  Inverse S-box substitution values in hexadecimal format.
inline constexpr uint8_t kSBox1[] = {
    ,
};

// Helper methods to get/put uint32 with big endianness
uint32_t GetU32(const uint8_t* addr);
void PutU32(uint32_t val, uint8_t* addr);

// Takes a four-byte input word and applies the S-box
uint32_t SubWord(uint32_t word);
// Takes a four-byte input word and applies the S-box-inv
uint32_t InvSubWord(uint32_t word);

// Takes a word [a0,a1,a2,a3] as input, performs a cyclic permutation, and
// returns the word [a1,a2,a3,a0]
uint32_t RotWord(uint32_t word);

}  // namespace cryptopals::aes

#endif  // CRYPTOPALS_AES_BASE_H_
